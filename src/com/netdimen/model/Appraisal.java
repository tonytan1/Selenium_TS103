...
public class Appraisal extends com.netdimen.abstractclasses.TestObject {
	...
	private static final String Exceeded_Expected_Performance = Config.getInstance()
			.getProperty("label.Exceeded_Expected_Performance");
	private static final String Perf_Goal_Plan = Config.getInstance()
			.getProperty("appraisal.sectionType.performance_goal_plan");
	private static final String Dev_Goal_Plan = Config.getInstance()
			.getProperty("appraisal.sectionType.development_goal_plan");
	private static final String Promt_Potential_Report = Config.getInstance()
			.getProperty("heading.plot_9box_promotion_potential_report");
	private static final String Retent_Risk_Report = Config.getInstance()
			.getProperty("heading.plot_9box_retention_risk_report");
	private static final String Appraisal_Saved_Msg = Config.getInstance()
			.getProperty("appraisal.msg.dataSavedComplete");
	private static final String Other_Achievements = Config.getInstance()
			.getProperty("label.Other_Significant_Achievements");
	private static final String Create_Goal = Config.getInstance()
			.getProperty("label.goal_create");
	...
	public void startAppraisal(WebDriver driver) {
		...
		if(!this.getGoals().equals("")){
			...
			for(Goal goal:goalArray){
				if(goal instanceof PerformanceGoal){
					by = By.xpath("//div[descendant::h4[text()='"+Perf_Goal_Plan+"']]/span/span[text()='"+Create_Goal+"']");	
				}else if(goal instanceof DevelopmentGoal){
					by = By.xpath("//div[descendant::h4[text()='"+Dev_Goal_Plan+"']]/span/span[text()='"+Create_Goal+"']");
				}
				Navigator.waitForAjax(driver, by);
				WebDriverUtils.clickButton(driver, by);
				goal.runCreateWithoutNavigation(driver);				
			}
		...
		by = By.xpath("//div[descendant::h4[text()='"+Other_Achievements+"']]/p/textarea");
		Navigator.waitForAjax(driver, by);
		...
		by = By.xpath("//tr[descendant::td[text()='"+Exceeded_Expected_Performance+"']]/td[1]/input[starts-with(@id,\"SC\")]");
		Navigator.waitForAjax(driver, by);
			...
			if(session.equalsIgnoreCase("GR")){
				...
				for (String goal_temp : goalsArray) {
					if(session.equalsIgnoreCase("PGP")){
						by = By.xpath("//div[descendant::h4[text()='"+Perf_Goal_Plan+"']]/table[@id='individual-goal-list']/tbody/tr/td[contains(text(),'"
								+ goal_temp + "')]");	
					}else{
						by = By.xpath("//div[descendant::h4[text()='"+Dev_Goal_Plan+"']]/table[@id='individual-goal-list']/tbody/tr/td[contains(text(),'"
								+ goal_temp + "')]");	
					}
					this.checkGoalInclusion(driver, by, criteria);
				}
...
//		String expectedResult = this.loadExpectedResult("Status2");
		by = By.xpath("//tr[descendant::td[3]/a[text()='"+this.getAppraisee().toLowerCase()+"'] and ./td[5][text()='"+this.getAppraisalID()+"']][1]/td[3]/a");
		//Navigator.explicitWait(1000);
		Navigator.waitForAjax(driver, by);	
		WebDriverUtils.clickLink(driver, by);
		WebDriverUtils.switchToPopUpWin(driver);
	...
	public void submitAppraisal_DA(WebDriver driver, String appraisalID){
		...
		by = By.xpath("//tr[descendant::td[3]/a[text()='"+ this.getAppraisee().toLowerCase() +"'] and ./td[5][text()='"+appraisalID+"'] and ./td[8][text()='"+expectedResult+"']][1]/td[3]/a");
		//Navigator.explicitWait(1000);
		Navigator.waitForAjax(driver, by);			
		WebDriverUtils.clickLink(driver, by);		
		WebDriverUtils.switchToPopUpWin(driver);
	...
	public void runCheckRateSumReport(WebDriver driver){
		...
		by = By.xpath("//tr[descendant::td[3]/a[text()='"+ user +"'] and ./td[5][text()='"+appraisalID+"'] and ./td[8][text()='"+expectedResult+"']][1]/td[3]/a");
		Navigator.waitForAjax(driver, by);	
		WebDriverUtils.clickLink(driver, by);
	...
	public void runCheck9BoxReport(WebDriver driver){
		...
		*/
		...
		//Navigator.explicitWait(1000);
		Navigator.waitForAjax(driver, by);	
		WebDriverUtils.select_selector(driver, by, Promt_Potential_Report);
...
//		Navigator.explicitWait();
		...
		by = By.id("reviewActionDropdown");
		WebDriverUtils.select_selector(driver, by, Retent_Risk_Report);
...
//		Navigator.explicitWait();
	...
	public void runCheckStatus(WebDriver driver) {
		...
		User user = new User(this.getDirectAppraisal(),"11111111");
		...
		by = By.xpath("//tr[descendant::td[3]/a[text()='"+this.getUID()+"'] and ./td[5][text()='"+this.getAppraisalID()+"'] and ./td[7][not(text()='-')]][1]/td[8]");
		Navigator.waitForAjax(driver, by);	
		status = WebDriverUtils.getText(driver, by);
		JUnitAssert.assertEquals(expectedStatus, status);
	}
	...
	private String loadExpectedResult(String criteria) {
		if(criteria_status == null){
			criteria_status = UIFunctionUtils
	...
	private void checkSaveMsg_UI(WebDriver driver) {
		...
		JUnitAssert.assertEquals(Appraisal_Saved_Msg, msg);
	}
}