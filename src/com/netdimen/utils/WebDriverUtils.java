...
public class WebDriverUtils {
	...
	public enum PopUpSelector {
		UserSelector, OrgSelector, CatalogSelector, InnerUserSelector,TopDownSelector,PermissionSelector
	}
	...
	public static boolean textPresentInPage(WebDriver driver, String text) {
		By by = By.xpath("//body");
		if(getHowManyByPresntInPage(by) > 0){
	 ...
	 * @return
	 */
	public static int getHowManyByPresntInPage(By by){ 
	...
	/**count how many webelement given with "by" object present in same page
	 * if no webelement is found then print out in console. Different from getHomeManyByPresentInPage(By by),
	 * this method will wait until element occurs if not found it, and stop execution if cannot find it before timeout.
	 * 
	 * @param by
	 * @return
	 */
	public static int getHowManyAjaxByPresentInPage(By by){
	...
	public static void switchToNestedFrame(WebDriver driver, ArrayList<By> bys) {
		...
		int size = -1;
		for (By by : bys) {
			size = getHowManyByPresntInPage(by);
			if (size == 1) {
	...
	/**
	 * Apply to search user, module, competency, job profile
	 * 
	 * @param driver
	 * @param keyword
	 */
	public static void searchSelect_Selector(WebDriver driver, String keyword) {
		if (!keyword.equals("")) {

			By by = By.id("SEARCHTEXT");
			int size = getHowManyByPresntInPage(by);
			if (size == 1) {
				WebDriverUtils.fillin_textbox(driver, by, keyword);
			} else {
				by = By.name("SEARCHTEXT");
				size = getHowManyByPresntInPage(by);
				if (size == 1) {
					WebDriverUtils.fillin_textbox(driver, by, keyword);
				} else {
			...
			if (size == 1) {
				...
				by = By.xpath("//input[@id='ADDSINGLE' and @value='>']");
				size = getHowManyByPresntInPage(by);
				if (size == 1) {
					WebDriverUtils.clickButton(driver, by);
				} else {
					by = By.xpath("//input[@id='aButton' and @value='>']");
					size = getHowManyByPresntInPage(by);
					if (size == 1) {
						WebDriverUtils.clickButton(driver, by);
					}
	 ...
	 * @param keywords
	 */
	public static void searchSelect_Selector(WebDriver driver, String[] keywords) {
	 ...
	 * @param keywords
	 */
	public static void general_Selector(WebDriver driver, WebElementWrapper web,  ArrayList<String> keywords){
		if(keywords.size() > 0){
			By by = null;
			WebElementWrapper searchElm = null;
			
			for (String keyword : keywords) {
				searchElm = WebElementManager.getInstance().getWebElementWrapper(web.getSheetName(), "1.SearchText");
				by = searchElm.constructBy(searchElm.getElementType(), searchElm.getElementValue());
				WebDriverUtils.fillin_textbox(driver, by, keyword);
				
				searchElm = WebElementManager.getInstance().getWebElementWrapper(web.getSheetName(), "1.SearchButton");
				by = searchElm.constructBy(searchElm.getElementType(), searchElm.getElementValue());
				WebDriverUtils.clickButton(driver, by);
				...
				searchElm = WebElementManager.getInstance().getWebElementWrapper(web.getSheetName(), "1.AddSingle");
				by = searchElm.constructBy(searchElm.getElementType(), searchElm.getElementValue());
				WebDriverUtils.clickButton(driver, by);
			}
					
			searchElm = WebElementManager.getInstance().getWebElementWrapper(web.getSheetName(), "1. SaveBtn");
			by = searchElm.constructBy(searchElm.getElementType(), searchElm.getElementValue());
			WebDriverUtils.clickButton(driver, by);
		}
	}
	...
	public static void searchSelect_Selector(WebDriver driver, PopUpSelector selector,  ArrayList<String> keywords){	
		...
		switch (selector){
			...
			case InnerUserSelector://CDC->Assessment->Peer Reviewer selector
				we= WebElementManager.getInstance().getWebElementWrapper(selector.toString(), "1.SearchText");
				WebDriverUtils.general_Selector(driver, we, keywords);
				break;
			case TopDownSelector://CDC->Development Goal-> module selector
				WebDriverUtils.switchToPopUpWin(driver);
				we= WebElementManager.getInstance().getWebElementWrapper(selector.toString(), "1.SearchText");
				general_Selector(driver, we, keywords);
				WebDriverUtils.switchToParentWin(driver);
				break;
			case PermissionSelector:
				UIFunctionUtils.setPermission_UI(driver, keywords.get(0));
				break;
			default:
				System.out.println("warning: selector not support yet in WebDriverUtils:"
			+"searchSelect_Selector(WebDriver driver, WebElementWrapper web)");	
		}
	}
...
//			WebDriverUtils.clickLink(driver, by);
	...
	public static void clickIfExist(WebDriver driver, By by) {
		int size = getHowManyByPresntInPage(by);
		if (size == 1) {
